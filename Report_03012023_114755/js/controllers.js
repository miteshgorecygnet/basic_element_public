...
myApp.config(function ($routeProvider) {
  $routeProvider
    ...
    // route for the contact page
    .when("/suite/:suiteIndex/case/:caseIndex/rerun/:rerunIndex", {
      templateUrl: "case.html",
      controller: "CaseController"
    });
});
...
myApp.controller("CaseController", [
  ...
  function ($scope, $http, $routeParams, $interval) {
    $scope.caseData = $routeParams.rerunIndex ?
      results.childObjects[$routeParams.suiteIndex].childObjects[
        $routeParams.caseIndex
      ].reRunLTestcases[$routeParams.rerunIndex] :
      results.childObjects[$routeParams.suiteIndex].childObjects[
      $routeParams.caseIndex
      ];
	
	$scope.recordingPath = results.metadata.recordingPath;
    $scope.consoleLogsetting = results.metadata.consoleLogsetting;
    $scope.recordingSetting = results.metadata.recordingSetting;
    $scope.iniTemp = 100;
    $scope.steps = $scope.caseData.childObjects;
    $scope.logs = [];
    $scope.logFlag = true;
    $scope.dateTime = '';
    $scope.requirementID = $scope.caseData.childObjects;
    ...
    $scope.expFile = function () {
      angular.forEach($scope.steps, function (value) {
        ...
        } else if (value.multipleLogsForCondtion.length > 0) {
          $scope.index = 1;
          angular.forEach(value.multipleLogsForCondtion, function (log) {
            $scope.seqNumber = value.sequencenumber ? value.sequencenumber.replace('.', '.' + $scope.index + " : ") : ' : ';
            $scope.logValue = log ? log.replace(/(\r\n|\n|\r)/gm, "") : '';
            if (value.action.name == 'Pink' || value.action.name == 'Blue' || value.action.name == 'Green' || value.action.name == 'Purple' || value.action.name == 'Black') {
              $scope.lastIndex = log.lastIndexOf(" ");
              $scope.logValue = log.slice(0, $scope.lastIndex + 1);
            }
            $scope.logs.push("Step#" + $scope.seqNumber + $scope.logValue + "\r\n");
      ...
      if ($scope.recordingSetting) {
		$scope.logs.push(results.metadata.recordingPath + "\r\n")
        $scope.logs.push(reportLang.recordingSettingMessage + "\r\n")
      }
    ...
    $scope.checkCallMethod = function (currentType, previousType) {
      if (
        typeof previousType != "undefined" &&
        (currentType.indexOf("CallMethod") > -1 || currentType.indexOf("CallTestCase") > -1)
      ) {
      ...
      } else if (currentType.indexOf("CallMethod") > -1 || currentType.indexOf("CallTestCase") > -1) {
        return true;
      } else {
        return false;
      }
    ...
    $scope.checkIterartionIsBlank = function (index) {
      if (
        ($scope.steps[index].type.indexOf("Iteration-") > -1 ||
          ($scope.steps[index].type.indexOf("CallMethod") > -1|| $scope.steps[index].type.indexOf("CallTestCase") > -1)) &&
